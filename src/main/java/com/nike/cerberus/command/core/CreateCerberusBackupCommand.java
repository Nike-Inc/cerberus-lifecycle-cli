...
@Parameters(commandNames = COMMAND_NAME,
        commandDescription = "Allows Cerberus operators to create a complete backup in S3 encrypted with KMS." +
                "That can be restored with the restore command")
public class CreateCerberusBackupCommand implements Command {
    ...
    @Parameter(names = { BACKUP_REGIONS_LONG_ARG, BACKUP_REGIONS_SHORT_ARG },
    ...
    @Inject
    public CreateCerberusBackupCommand(CerberusAdminClientFactory cerberusAdminClientFactory,
    ...
    @Override
    public void execute() {
        logger.info("Starting Cerberus Backup");

        List<String> regionsToStoreBackups = backupRegions;
        ...
        if (sdbCount < 1) {
            ...
            logger.info(String.format("CMS returned that there are %s SDBs to back up.", sdbMetadataList.size()));
        }

        CerberusSdbMetadata cerberusSdbMetadata = new CerberusSdbMetadata();
        for (SafeDepositBox sdb : sdbMetadataList) {
            logger.info(String.format("Backing up %s", sdb.getName()));
        ...
        regionsToStoreBackups.forEach(region -> {
            ...
            logger.info("Backup for region: {} complete, bucket: {}, prefix: {}", region, bucket, prefix);
        });
    }
    ...
    public boolean isRunnable() {
        if (configStore.getBackupAdminIamPrincipals().isEmpty()) {
            logger.error("Backup Admin Principals have not been set please run " + SetBackupAdminPrincipalsCommand.COMMAND_NAME);
            return false;
        }
        return true;
    }
    ...
    private String provisionBackupBucketForRegion(String region) {
        ...
        logger.info("Created bucket in region: {} with name: {}", region, bucketName);
        return bucketName;
    }
    ...
    private String provisionKmsCmkForBackupRegion(String region) {
        ...
        configStore.getBackupAdminIamPrincipals().forEach( principal -> {
            logger.debug("Adding principal: {} to the CMK Policy for region {}", principal, region);
        ...
        logger.debug("Creating key for region {} with policy {}", region, policyString);
        ...
        logger.info("Created new backup KMS CMK with id: {} for region: {}", keyId, region);
